...
- [ ] 3.4 Confirm opened bundles (with and without provenance, view-only included) show the overlay from `result.size` / `result.originPx`. Verify: dev server — open a saved `.sprite`, toggle Box, overlay aligns with the baked passes.

## 4. Final verification

- [ ] 4.1 Run `npx tsc --noEmit && npm run build` and the full `src/bake/scratch-verify.ts` suite; fix any regressions. Verify: both pass clean.
- [ ] 4.2 Manual browser pass over the spec scenarios: height↔scale sync, cap warning, overlay in all four views, per-tab toggle independence, view-only bundle overlay, height not persisted after save/re-bake. Verify: each scenario in the two delta specs behaves as written; note any deviation as a follow-up.
