...
- [x] 3.2 Re-verify in the browser (`npm run dev` → `/scratch-verify.html`): bake→set-origin and set-origin→bake produce identical sprites and aligned overlays for a primitive and a glTF model, N and E slots, grounding shadow on and off; bundle hashes for default-origin documents unchanged.
