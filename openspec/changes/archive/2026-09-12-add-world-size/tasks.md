...
- [x] 5.2 Browser pass (user-run, `npm run dev`): new-world dialog defaults/clamp/cancel; create 24 × 8 world; resize 12 × 12 → 24 × 8 and back; shrink with placements outside bounds; save + reload a sized world and a `/6` file; place-in-world button gone; brush loading from `sprites/` still works
