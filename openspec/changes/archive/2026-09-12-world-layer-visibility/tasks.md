...
- [x] 2.3 Browser check (user-run, `npm run dev`): toggling each of the three layers hides/shows it immediately — sprites/meshes take their baked grounding shadows with them, hiding the ground removes the contact shadows too, and re-showing restores everything unchanged; with all layers hidden the background stays clean
...
- [x] 4.4 Browser check (user-run): dropdown opens/closes via button, outside click, and Escape; row toggles update the check states and the viewport in the same frame; the tab does not turn dirty; switching tabs and back preserves visibility; undo/redo never toggles it; a saved+reopened world starts all-visible
