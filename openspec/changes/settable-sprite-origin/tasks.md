...
## 7. Follow-up fixes (origin "off" in E/W — user report)

- [x] 7.1 Realtime 3D marker applied the slot rotation twice: `setOrigin` pre-mapped the anchor with `slotAnchorPoint` even though the overlay group already carries the `applySlotModelRotation` rotation. Fix: marker local position is the raw authored anchor; the group maps it into the slot frame (matching the model transform exactly). Verified numerically against the model transform in all four slots (Node probe); browser marker position left to the user's harness pass. Verify: Node probe deltas 0; `npm run build`.
- [x] 7.2 `decodeBundle` restored every extra view with the N view's box (`m.cube.size`), so E/W (x/z swap) restored the wrong `result.size` — the overlay cross and `setBakeOrigin`'s re-projection ran over the wrong frame (S coincided). Fix: derive each extra view's rotated box via `yawRotatedBoxSize(m.cube.size, slotYawDeg(slot))`. scratch-verify 3d now bakes the slab (swap-sensitive) with an anchor and asserts the restored box, originPx, and anchor re-projection. Verify: `npm run verify:bundles`, `npm run build`; browser decode check in `/scratch-verify.html`.
- [x] 7.3 Gates re-run after the fixes: `npm run build` clean, `npm run verify:bundles` 34/34; Node probe: 2D overlay, realtime marker, and restored-size paths all match the model-transform ground truth. Verify: commands exit clean.
