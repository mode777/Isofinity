...
plain JSON carrying a format marker (`isoinfinity-world/6`) so the layout
...
(optional — omitted when no material is selected) and the ground tile scale
(optional — omitted at the default scale), and the world's selected HDRI
file name (optional — omitted when the environment is inherited from sprite
bake provenance). The user SHALL be able to name the world, with a sensible
...
through `isoinfinity-world/6` files: a `/1` placement, or a `/2` placement
...
a direction field SHALL restore facing north; a stored direction SHALL be
rejected as malformed unless it names a view slot (`n`, `e`, `s`, or `w`).
...
a status notice while the rest of the scene loads — and the tile scale and
HDRI SHALL restore when present and valid. Older files without ground
fields SHALL load unchanged, leaving the ground in its default state.
