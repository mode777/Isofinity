...
carrying a format marker (`isoinfinity-world/4`) so the layout can evolve;
...
north-facing placement may omit it). The file SHALL also record the ground
state: the selected ground material's file name (optional — omitted when no
material is selected) and the ground tile scale (optional — omitted at the
default scale), and the world's selected HDRI file name (optional — omitted
when the environment is inherited from sprite bake provenance). The user
...
`isoinfinity-world/1` through `isoinfinity-world/4`
...
unless it names a view slot (`n`, `e`, `s`, or `w`). Ground state SHALL
restore when present in a `/4` file: the material SHALL be resolved against
the workspace's `materials/` folder — a material that cannot be resolved or
parsed SHALL be skipped with a status notice while the rest of the scene
loads — and the tile scale and HDRI SHALL restore when present and valid.
Older files without ground fields SHALL load unchanged, leaving the ground
in its default state. Placements whose sprite
