...
- **Directional shadow** (ADR 0015): the `key` term is scaled by a per-pixel
  ...
  normal-offset bias, larger at grazing incidence), which is what keeps a
  large light-facing structure from self-shadowing. `npm run
...
the light pass's position reconstruction); blending is straight alpha with
the SRC_ALPHA factor read from each output's own alpha (ADR 0020): sprite
fragments ship surface data with alpha 1 so RT1/RT2 replace (a silhouette
fragment writes its own surface data — never a coverage-weighted mix with
the surface behind) while RT0's coverage alpha composites; zero-weight
outputs preserve, with the
...
5. **Sprites** — one instanced quad per placed object (per-instance quad
   ...
   RT1/RT2 (shipped with alpha 1, so they replace — a silhouette fragment
   carries its own surface data, never a coverage-weighted mix with what
   is behind it, so the deferred pass shades it as the object, ADR 0020).
