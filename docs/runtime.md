...
2. **Sprites** — one instanced quad per placed object, painter-sorted by
   `dot(cellCenter, viewDir)` (far → near), alpha-blended using the baked
...
Left-click/drag places the selected tool, right-click erases, toolbar or
eraser tool selects. Cell picking inverts the shared projection analytically
(`screenToGround`), no hit-testing.
...
- `src/runtime/world.ts` — grid state + painter sort
